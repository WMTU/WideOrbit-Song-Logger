...
public class LoggingWidget extends BasicWidget implements ActionListener {
  ...
  private static final Logger log = LogManager.getLogger( LoggingWidget.class );
  ...
  {
    getToolbar().setTitle( "Song Logging" );
    ...
    gbc = createGbc( 0, 2 );
    locationLabel = new JLabel( "Library", JLabel.RIGHT );
    ...
    gbc = createGbc( 1, 2 );
    String[] locations = { "CD Library", "Vinyl", "Personal" };
   ...
   * to a playlist entry whose search populates the results of this AsyncCallback
   */
   ...
   * corresponding to a playlist entry whose search populates the results of this
   * AsyncCallback
   */
  private AsyncCallback<MediaAsset, LibraryError> assetFromEntryCheckChangedEntry = new AsyncCallback<MediaAsset, LibraryError>() 
  {
    public void onSuccess( MediaAsset asset ) {
      ...
      boolean success = sendLog( song, artist, album, "CD Library", "", asset.getId().value() );
